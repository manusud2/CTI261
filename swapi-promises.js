...
function getJSON(url) {
  ...
  function renderlist(ships, listElement) {
    // I decided to use a table to display my list of ships. The list Element is that table and it has 2 children: thead and tbody...we need to put our ships into tbody...so I reference the second child.
  ...
  function showShips(url = 'https://swapi.co/api/starships/') 
  {
    getShips(url).then(function(data) 
    {
      ...
      renderlist(results, listElement);
